"""
File: project1.py
Description: CSCI 635 Project 1
Language: python3.8
...
def read_data(filename, size, model_index):
    ...
    # a 1D average pool layer used to reduce the input size
    avg_pool_1d = tf.keras.layers.AveragePooling1D(pool_size=25, strides=25, padding='valid')
    ...
    for example in dataset.take(size):
        ...
        if example["instrument_family"] != 9:
            # trim the final part where values are all 0's
            tensor = tf.slice(example["audio"], [0], [51200])
            tensor = tf.reshape(tensor, [1, 51200, 1])
    ...
    # create a dummy dimension at axis 3
    examples = tf.expand_dims(examples, axis=3, name=None)
...
def train_model(train_x, train_y, valid_x, valid_y, model_index):
    ...
    # our first CNN for instrument classifier
    if model_index == 1:
        my_model.add(
            tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 1), activation='relu', input_shape=train.shape[1:]))
        my_model.add(tf.keras.layers.AveragePooling2D(pool_size=(3, 1), padding="same"))
        my_model.add(tf.keras.layers.Dropout(rate=0.2))
        my_model.add(tf.keras.layers.Conv2D(filters=64, kernel_size=(5, 1), activation='relu'))
        my_model.add(tf.keras.layers.AveragePooling2D(pool_size=(5, 1), padding="same"))
        my_model.add(tf.keras.layers.Dropout(rate=0.2))
        my_model.add(tf.keras.layers.Conv2D(filters=128, kernel_size=(5, 1), activation='relu'))
    ...
    # our second CNN for instrument classifier
    elif model_index == 2:
        my_model.add(
            tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 1), activation='relu', input_shape=train.shape[1:],
...
def make_plots(my_model, test_x, test_y):
    ...
    # plt.savefig('images/1DExample.png')
    plt.show()
    ...
    plt.legend()
    # plt.savefig('Project1/images/nn1_class_example_1.png')
    plt.show()
    plt.figure(2)
    ...
    plt.title('Reconstructed Waveforms')
    plt.legend()
    # plt.savefig('Project1/images/nn1_class_example_2.png')
    plt.show()
    plt.figure(3)
    ...
    plt.title('Reconstructed Waveforms')
    plt.legend()
    # plt.savefig('Project1/images/nn1_class_example_3.png')
    plt.show()
    plt.figure(4)
    ...
    plt.title('Reconstructed Waveforms')
    plt.legend()
    # plt.savefig('Project1/images/nn1_class_example_4.png')
    plt.show()
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm,
                                  display_labels=['Bass', 'Brass', 'Flute', 'Guitar', 'Keyboard', 'Mallet', 'Organ',
                                                  'Reed', 'String', 'Vocal'])
    disp.plot()
    plt.title("Confusion Matrix Network 2")
    # plt.savefig('Project1/images/confusion_matrix.png')
    plt.show()
    ...
    plt.legend()
    # plt.savefig('Project1/images/nn2_hp_example_1.png')
    plt.show()
    plt.figure(6)
    ...
    plt.title('Reconstructed Waveforms High Probability')
    plt.legend()
    # plt.savefig('Project1/images/nn2_hp_example_2.png')
    plt.show()
    plt.figure(7)
    ...
    plt.title('Reconstructed Waveforms High Probability')
    plt.legend()
    # plt.savefig('Project1/images/nn2_hp_example_3.png')
    plt.show()
    plt.figure(8)
    ...
    plt.title('Reconstructed Waveforms High Probability')
    plt.legend()
    # plt.savefig('Project1/images/nn2_hp_example_4.png')
    plt.show()
    ...
    plt.legend()
    # plt.savefig('Project1/images/nn2_mixed_example_1.png')
    plt.show()
    plt.figure(10)
    ...
    plt.title('Reconstructed Waveforms Mixed Probability')
    plt.legend()
    # plt.savefig('Project1/images/nn2_mixed_example_2.png')
    plt.show()
    plt.figure(11)
    ...
    plt.title('Reconstructed Waveforms Mixed Probability')
    plt.legend()
    # plt.savefig('Project1/images/nn2_mixed_example_3.png')
    plt.show()
    plt.figure(12)
    ...
    plt.title('Reconstructed Waveforms Mixed Probability')
    plt.legend()
    # plt.savefig('Project1/images/nn2_mixed_example_4.png')
    plt.show()
